...
END AS rfm_bucket, ab.activity_segment, COUNT(ab.mobile), SUM( ab.`Total Spends`) sales, 
...
SELECT * FROM fact_customer_rfm LIMIT 5;

SELECT DISTINCT `last shopped store` FROM fact_customer_rfm;
SELECT * FROM program_single_view;


SELECT DISTINCT `last shopped store` FROM program_single_view;


SELECT store_type,SUM(`Total Transactions`) bills, 
SUM(`1_year_sales`) 1_year_sales ,SUM(`1_year_bills`) 1_year_bills 
FROM dummy.camp_coco_fofo_store a LEFT JOIN program_single_view b ON a.storecode=b.`last shopped store`
GROUP BY 1;