...
WHERE ab.recency > 14 AND b.storetype1 = 'COCO' AND psv.`last shopped store` NOT IN ('ecom', 'demo')
AND ab.mobile COLLATE utf8mb4_0900_ai_ci IN (SELECT DISTINCT mobile FROM member_report WHERE smsreachability='true')
GROUP BY 1,2;

-- QC
SELECT SUM( ab.`Total Spends`) sales, 
SUM(ab.`Total Transactions`) bills, SUM(`1_year_sales`) 1_year_sales ,SUM(`1_year_bills`) 1_year_bills
FROM fact_customer_rfm ab
JOIN program_single_view psv ON ab.mobile COLLATE utf8mb4_0900_ai_ci = psv.mobile
LEFT JOIN store_master b ON psv.`last shopped store` = b.storecode
WHERE ab.recency > 14 AND b.storetype1 = 'fofo' AND psv.`last shopped store` NOT IN ('ecom', 'demo')
AND ab.mobile COLLATE utf8mb4_0900_ai_ci IN (SELECT DISTINCT mobile FROM member_report WHERE smsreachability='true');
...
JOIN program_single_view psv ON ab.mobile COLLATE utf8mb4_0900_ai_ci = psv.mobile
LEFT JOIN store_master b ON psv.`last shopped store` = b.storecode
WHERE ab.recency > 14 AND b.storetype1 = 'FOFO' AND psv.`last shopped store` NOT IN ('ecom', 'demo','corporate')
AND ab.mobile COLLATE utf8mb4_0900_ai_ci IN (SELECT DISTINCT mobile FROM member_report WHERE smsreachability='true')
GROUP BY 1,2;
...
JOIN program_single_view psv ON ab.mobile COLLATE utf8mb4_0900_ai_ci = psv.mobile
LEFT JOIN store_master b ON psv.`last shopped store` = b.storecode
WHERE ab.recency > 14 AND psv.`last shopped store` = 'ecom' AND psv.`last shopped store` NOT IN ('demo','corporate')
AND ab.mobile COLLATE utf8mb4_0900_ai_ci IN (SELECT DISTINCT mobile FROM member_report WHERE smsreachability='true')
GROUP BY 1,2;
...
JOIN program_single_view psv ON ab.mobile COLLATE utf8mb4_0900_ai_ci = psv.mobile
LEFT JOIN store_master b ON psv.`last shopped store` = b.storecode
WHERE ab.recency > 14 AND psv.`last shopped store` <> 'demo' AND psv.`last shopped store`<> 'corporate'
AND ab.mobile COLLATE utf8mb4_0900_ai_ci IN (SELECT DISTINCT mobile FROM member_report WHERE smsreachability='true')
GROUP BY 1,2;
...
SELECT COUNT(*) FROM member_Report WHERE MONTHNAME(dateofbirth) = 'November'
...
(SELECT mobile, CASE WHEN `total transactions` = 0 THEN enrolledstore
                     WHEN `total transactions` > 0 THEN `last shopped store` END AS store
FROM program_single_view WHERE MONTHNAME(dateofbirth) = 'November' 
AND mobile IN (SELECT DISTINCT mobile FROM member_report WHERE smsreachability='true')
GROUP BY 1)a 
JOIN store_master d
ON a.store = d.storecode
...
FROM program_single_view WHERE enrolledon BETWEEN '2024-07-01' AND '2025-09-30'
...
(SELECT mobile, enrolledstore AS store
FROM program_single_view WHERE enrolledon BETWEEN '2025-07-01' AND '2025-09-30'
AND `total transactions` = 0 AND `last shopped date` IS NULL AND enrolledstore NOT LIKE '%demo%'
AND enrolledstore NOT LIKE '%corporate%'
AND mobile IN (SELECT DISTINCT mobile FROM member_report WHERE smsreachability='true')
GROUP BY 1)a 
