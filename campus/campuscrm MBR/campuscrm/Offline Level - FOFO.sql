...
FROM(
  ...
  JOIN store_master b USING(storecode) 
  WHERE a.TxnDate BETWEEN @startdate AND @enddate
--   AND a.mobile IN (SELECT mobile FROM member_report)
    AND a.storecode NOT IN ('Demo','ecom','corporate')
    AND a.billno NOT LIKE '%test%' 
    AND a.billno NOT LIKE '%roll%' AND a.amount>0
...
FROM(
  ...
  FROM txn_report_accrual_redemption a 
  JOIN store_master b USING(storecode) 
  WHERE a.TxnDate BETWEEN @startdate_1m AND @enddate_1m
--   AND a.mobile IN (SELECT mobile FROM member_report)
    AND a.storecode NOT IN ('Demo','ecom','corporate')
    AND a.billno NOT LIKE '%test%' 
    AND a.billno NOT LIKE '%roll%' AND a.amount>0
...
FROM(
  ...
  FROM txn_report_accrual_redemption a 
  JOIN store_master b USING(storecode) 
  WHERE a.TxnDate BETWEEN @startdate- INTERVAL 1 YEAR  AND @enddate- INTERVAL 1 YEAR 
--   AND a.mobile IN (SELECT mobile FROM member_report)
    AND a.storecode NOT IN ('Demo','ecom','corporate')
    AND a.billno NOT LIKE '%test%' 
    AND a.billno NOT LIKE '%roll%' AND a.amount>0
...
AND itemnetamount>0
AND modifiedstorecode IN (SELECT DISTINCT storecode FROM store_master WHERE storetype1= 'FOFO')
UNION 
SELECT 'Aug-25',
COUNT(DISTINCT uniquebillno)nonloyalty_bills,SUM(itemnetamount)NOn_loyalty_sales
FROM sku_report_nonloyalty
